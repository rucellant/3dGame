#include "stdafx.h"
#include "..\Headers\Particle_Dead.h"
#include "Shader.h"
#include "Texture.h"
#include "Renderer.h"
#include "Transform.h"
...
CParticle_Dead::CParticle_Dead(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CParticle(pGraphic_Device)
{
}

CParticle_Dead::CParticle_Dead(const CParticle_Dead & rhs)
	: CParticle(rhs)
{
}

_bool Compare_View(const CParticle_Dead::ATTRIBUTE & a, const CParticle_Dead::ATTRIBUTE & b)
{
	_matrix	matView;
...
HRESULT CParticle_Dead::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CParticle_Dead::Ready_GameObject_Clone(void * pArg)
{
	...
	m_pManagement->Push_GameObject(g_eScene, L"Layer_Particle_Dead", this);

	return NOERROR;
}

_int CParticle_Dead::Update_GameObject(_double TimeDelta)
{
	m_TimeAcc += TimeDelta;
	...
	for (auto& Attribute : m_ParticleLst)
	{
		...
		if (Attribute.fLifeTime <= Attribute.fAge)
			...
			Attribute.vPosition += (Attribute.vDir * _float(TimeDelta) * 2.f);
	}

	return _int();
}

_int CParticle_Dead::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CParticle_Dead::Render_GameObject()
{
...
HRESULT CParticle_Dead::Activate(_vec3 vOrigin)
{
	m_vOrigin = vOrigin;
...
HRESULT CParticle_Dead::Add_Component(void * pArg)
{
	// For. Com_Shader
	...
	// For. Com_Texture
	if (CGameObject::Add_Component(SCENE_STATIC, L"Component_Texture_Particle_Dead", L"Com_Texture", (CComponent**)&m_pTextureCom))
		return E_FAIL;
...
HRESULT CParticle_Dead::SetUp_ConstantTable()
{
...
HRESULT CParticle_Dead::Render(_uint iPassIndex)
{
...
HRESULT CParticle_Dead::Render_Particle()
{
...
CParticle_Dead * CParticle_Dead::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CParticle_Dead* pInstance = new CParticle_Dead(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CParticle_Dead");
...
CGameObject * CParticle_Dead::Clone_GameObject(void * pArg)
{
	CParticle_Dead* pInstance = new CParticle_Dead(*this);
